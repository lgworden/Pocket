...
//
// Modal-less on purpose: it's one branch of AddToClosetModal (the other being
// AddPieceForm), so the sheet chrome and title live with the parent.
export default function LogFitForm({
  active,
  onBack,
  onSaved,
}: {
    ...
    return (
      <div className="space-y-4">
        <button
          ...
          className="text-xs font-ui text-slate hover:text-ink"
        >
          ← back to your fit
        </button>
                    ...
                    >
                      <div
                        className={`relative aspect-square rounded-lg overflow-hidden bg-blue/10 border transition-colors ${
                          isSelected ? "border-ink border-2" : "border-slate/15"
                        }`}
                      >
                        {item.photos[0] ? (
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-ink/60">
        A photo of the whole look. Tag the pieces you&apos;re wearing and their wear
        counts update on their own.
      </p>
      ...
      {/* Favourite — feeds "shuffle favs" on the Today screen. */}
      <label className="flex items-start gap-2 text-sm text-ink">
        ...
        />
        <span>
          Save this fit to my favs
          <span className="block text-xs text-slate/70">
            shuffle favs will reach for it more often
          </span>
        </span>
      </label>
